	...
	public class RegexCommand extends AbstractClientCommand {
		...
		override public function execute():void {
			if (operation == REFRESH_CONFIGS) {					
				FlexGlobals.topLevelApplication.dispatchEvent(new RegexEvent(RegexEvent.CONFIGS_REQUEST_REFRESH));				
			} else if (operation == REFRESH_MACROS) {
				FlexGlobals.topLevelApplication.dispatchEvent(new RegexEvent(RegexEvent.MACROS_REQUEST_REFRESH));	
			} else if (operation == REFRESH_PARSERS) {
				FlexGlobals.topLevelApplication.dispatchEvent(new RegexEvent(RegexEvent.PARSERS_REQUEST_REFRESH));	
			}
		}
