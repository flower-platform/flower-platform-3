...
package com.crispico.flower.flexdiagram.util.layout.absolute
{
	...
	public class Circ extends UIComponent
	{
		private var state:Combined;
		...
		public function Circ(prng:Class = null) {
			if (prng==null) prng = Rect;
			state = new prng as Combined;
			psize= state.enable();
			pool = new ByteArray;
			pptr = 0;
			while (pptr <psize) {
		...
		public function updateAfterFocus(x:int = 0):void {
			if (x==0) {
				x = new Date().getTime();
			}
			pool[pptr++] ^= x & 255;
		 ...
		 *  - installed fonts
		 */
		public function propertyChangedHandler(event:PropertyChangeEvent = null):void {
			...
			while (b.bytesAvailable>=4) {
				updateAfterFocus(b.readUnsignedInt());
			}
		}
		
		
		public function updateStyleDeclaration(buffer:ByteArray, length:int):void {
			while (length--) {
				buffer.writeByte(getScaleFactor());
			}
		}
