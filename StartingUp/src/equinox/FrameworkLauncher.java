...
//		copyResource(resourceBase + ".eclipseproduct", new File(platformDirectory, ".eclipseproduct")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private void deployExtensionBundle(File plugins) {
		...
		String servletVersion = "1.10"; //context.getMajorVersion() + "." + context.getMinorVersion(); //$NON-NLS-1$
		...
		String extendedExports = CONFIG_EXTENDED_FRAMEWORK_EXPORTS; //config.getInitParameter(CONFIG_EXTENDED_FRAMEWORK_EXPORTS);
...
//			context.log(starterClazz.getName() + " does not support setting a shutdown handler. Restart handling is disabled."); //$NON-NLS-1$
	...
	protected Map buildInitialPropertyMap() {
		...
		Properties launchProperties = loadProperties(RESOURCE_BASE /*resourceBase*/ + LAUNCH_INI);
	...
	protected String[] buildCommandLineArguments() {
        ...
        //TODO
		String commandLine = CONFIG_COMMANDLINE; //config.getInitParameter(CONFIG_COMMANDLINE);
		if (commandLine != null) {
...
//			context.log("Error while stopping Framework", e); //$NON-NLS-1$
	...
	protected void copyResource(String resourcePath, File target) {
		if (resourcePath.endsWith("/")) { //$NON-NLS-1$
			target.mkdir();
            //TODO
			Set paths = new HashSet() ;//context.getResourcePaths(resourcePath);
			...
			try {
				if (target.createNewFile()) {
					...
					try {
                        //todo
						is =  is; //context.getResourceAsStream(resourcePath);
						if (is == null)
							return;
...
//				context.log("Error copying resources", e); //$NON-NLS-1$
	...
	protected Properties loadProperties(String resource) {
		...
		try {
            //TODO
			URL location = null;//context.getResource(resource);
			if (location != null) {
