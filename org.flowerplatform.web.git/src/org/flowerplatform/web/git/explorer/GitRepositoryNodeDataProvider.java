...
import org.flowerplatform.web.explorer.OrganizationChildrenProvider;
...
public class GitRepositoryNodeDataProvider implements INodeDataProvider {
	...
	private String getLabel(Object node) {		
		...
		if (GitRepositoryChildrenProvider.NODE_TYPE_LOCAL_BRANCHES.equals(nodeType)) {
			return "Workspaces";
		} else if (GitRepositoryChildrenProvider.NODE_TYPE_REMOTE_BRANCHES.equals(nodeType)) {
			return "Src Repositories";
		} else if (GitRepositoryChildrenProvider.NODE_TYPE_TAGS.equals(nodeType)) {
			return "Issues";
		} else if (GitRepositoryChildrenProvider.NODE_TYPE_REMOTES.equals(nodeType)) {
			return "Wiki";
		} else if (GitRepositoryChildrenProvider.NODE_TYPE_WDIRS.equals(nodeType)) {
			return "Wiki";
		} else {
			return null;
		}
