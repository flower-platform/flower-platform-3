...
public class CreateRemoteFolderTest {
	...
	@Test
	public void testCreateFolder() throws InstantiationException,
			IllegalAccessException, TeamException {
		...
		actionResult = SvnService.getInstance().createRemoteFolder(context,
				parentPathFragment, "createT1", "lala");
		assertEquals(true, actionResult);
		// end of Test 1

		// Test 2 => create a new remote folder inside an existing folder from
		// the repository; it will work if Test 1 works
		parentPathFragment = getArrayOfPathFragmentsFromStringArgs(
				"explorerTreeStatefulService|Explorer1", "r", "hibernate",
				"organization", ".svn-repositories", "svnRepositories",
				"svn://csp1/flower2", "svnRepository", "createT1", "svnFolder");
		actionResult = SvnService.getInstance().createRemoteFolder(context,
				parentPathFragment, "createT2", "lala");
		assertEquals(true, actionResult);
		// end of Test 2

		// Test 3 => create a folder with an existing name; if Tests 1 and 2
		// worked, this should fail, because the item already exists
		// in the file system
		// parentPathFragment = getArrayOfPathFragmentsFromStringArgs(
		// "explorerTreeStatefulService|Explorer1", "r", "hibernate",
		// "organization", ".svn-repositories", "svnRepositories",
