 ...
 */
public class SvnRepositoryTreeStatefulService extends AbstractTreeStatefulService {

	
	public static String ORGANIZATION_KEY = "organization";
	
	private static Logger logger = LoggerFactory.getLogger(SvnRepositoryTreeStatefulService.class);
	...
	public SvnRepositoryTreeStatefulService() throws CoreException {
		setStatefulClientPrefixId("Working Directory");
		
		{
			// workingDirChildrenProvider
			IConfigurationElement[] configurationElements = Platform.getExtensionRegistry().getConfigurationElementsFor("org.flowerplatform.web.svn.svnRepositoryChildrenProvider");
		...
		
		{
			...
			IConfigurationElement[] configurationElements = Platform.getExtensionRegistry().getConfigurationElementsFor("org.flowerplatform.web.svn.svnRepositoryNodeDataProvider");
			for (IConfigurationElement configurationElement : configurationElements) {
	...
	protected String getNodeType(TreeNode treeNode) {
		if (treeNode.getPathFragment() == null) {
			return SvnNodeType.NODE_TYPE_SVN_REPOSITORIES;
		} else {
	...
	public String getLabelForLog(Object node, String nodeType) {
		if (SvnNodeType.NODE_TYPE_SVN_REPOSITORIES.equals(nodeType)) {
			return "root";
		}
		INodeDataProvider provider = nodeDataProviders.get(nodeType);
		if (provider == null) {
	...
	public Object getNodeByPath(List<PathFragment> fullPath, GenericTreeContext context) {
		...
		File orgFile = ProjectsService.getInstance().getOrganizationDir(organization);	
		Pair<File, String> svnRepositories = new Pair<File, String>(orgFile, SvnNodeType.NODE_TYPE_SVN_REPOSITORIES);
	...
	public TreeNode openNodeInternal(CommunicationChannel channel, String statefulClientId, List<PathFragment> fullPath, Map<Object, Object> context) {
		...
		if (fullPath != null) {
			...
			fullPath.add(0, new PathFragment(".svn-repositories", SvnNodeType.NODE_TYPE_SVN_REPOSITORIES));
