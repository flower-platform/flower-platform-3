...
package org.flowerplatform.mindmap.controller {
	...
	public class NodeController extends ControllerBase implements IMindMapModelController {
		...
		public function setExpanded(model:Object, value:Boolean):void {
			if (value) {
				MindMapPlugin.getInstance().service.getChildrenForNodeId(Node(model).id, this, getChildrenForNodeIdCallbackHandler);
			} else {				
				disposeModel(model);				
			}		
		}
		...
		private function getChildrenForNodeIdCallbackHandler(result:ArrayCollection):void {
			var node:Node = MindMapPlugin.getInstance().service.getNodeFromId(Node(Diagram(diagramShell.rootModel).rootNode), result[0]);
			
			for each (var child:Node in ArrayCollection(result[1])) {
				child.parent = node;
			}
			node.children = result[1];
